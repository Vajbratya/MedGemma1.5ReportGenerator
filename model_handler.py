...
def generate_report_pipeline(
    *,
    ...
    # Chunk mais conservador (limite menor de tokens) para reduzir variância e custo.
    chunk_gen = GenerationParams(
        max_new_tokens=min(int(gen.max_new_tokens), 220),
        temperature=max(float(gen.temperature), 0.2) if gen.do_sample else 0.0,
        top_p=float(gen.top_p),
        top_k=int(gen.top_k),
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        out, in_len = generate_images_to_text(manager=manager, images=chunk, prompt=chunk_prompt, gen=chunk_gen)
