...
from reporting import Language, ReportTemplate, build_debug_footer, sanitize_findings, sanitize_phi_text
from vram import (
    QuantizationMode,
    format_vram_summary,
    get_gpu_info,
...
def process_dicom_file(
    ...
    try:
        if not file_path:
            return "Nenhum arquivo enviado.", "", [], gr.update(choices=[], value=[])
        ...
        status = f"Processado: {len(images)} imagens ({modality})"
    ...
    with gr.Blocks(title="Gerador de Laudo DICOM (MedGemma 1.5) — Enhanced", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("Processamento de imagens", open=True):
                    ...
                    max_slices_slider = gr.Slider(
                        ...
                        info="0 = amostragem global (não recomendado quando há muitas séries). Reduza para economizar VRAM.",
                    )
                    image_size_slider = gr.Slider(
                        ...
                        info="Menor = menos VRAM, menos detalhe",
                    )

                    gr.Markdown("**Windowing (para TC / RX)**")
                    use_auto_window = gr.Checkbox(
                        label="Usar janela automática (metadados do DICOM)",
                        value=True,
                    )
                    with gr.Row():
                    ...
                    auto_fit_fraction = gr.Slider(
                        ...
                        info="O auto-fit usa VRAM total (não a livre). Reduza se você estiver com outras coisas abertas na GPU.",
                    )

                status_output = gr.Textbox(label="Status", interactive=False)
                study_info_box = gr.Textbox(
                    label="Info do estudo & estimativa de memória",
                    interactive=False,
                    lines=18,
                )
                ...
                with gr.Accordion("Avançado: modelo (recarregar, se precisar)", open=False):
                    quantization = gr.Dropdown(
                        ...
                        info="4bit/8bit precisa de bitsandbytes. Se não estiver instalado, cai para none.",
                    )
                    dtype = gr.Dropdown(
                        label="DType",
                    ...
                    attn_impl = gr.Dropdown(
                        label="Implementação de atenção",
            ...
            with gr.Column(scale=1):
                ...
                gr.Markdown("*Imagens amostradas que serão enviadas para o modelo*")
            ...
            with gr.Column(scale=1):
                ...
                clinical_history = gr.Textbox(
                    ...
                    lines=2,
                    placeholder="Ex.: febre e tosse; avaliar pneumonia; dor abdominal...",
                )
                additional_instructions = gr.Textbox(
                    label="Instruções adicionais (opcional)",
                    lines=2,
                    placeholder="Ex.: seja objetivo; foque em pulmões; use impressão numerada...",
                )
                prompt_override = gr.Textbox(
                    label="Prompt manual (avançado; opcional)",
                    lines=3,
                    placeholder="Deixe em branco para usar o construtor de prompt estruturado.",
                )

                sanitize_phi_checkbox = gr.Checkbox(
                    label="Sanitizar PHI/PII (recomendado)",
                    value=True,
                    info="Remove possíveis nomes/IDs/contatos do texto antes de enviar para o modelo e antes de salvar.",
                )
                ...
                pipeline_mode = gr.Radio(
                    ...
                    choices=[
                        "single",
                        "single + refine (bom quando a qualidade varia)",
                ...
                chunk_size = gr.Slider(
                    ...
                    value=16,
                    step=1,
                    label="Tamanho do chunk (imagens por inferência) — só nos modos chunked",
                )
