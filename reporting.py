...
def _extract_first_json_object(text: str) -> Optional[str]:
    ...
    # Fast path: if it already looks like a single JSON object.
    text = text.strip()
...
def sanitize_phi_text(text: str, *, language: Language = "Português (Brasil)") -> Tuple[str, List[str]]:
    ...
    if _LONG_DIGITS_RE.search(s):
        # Evita destruir valores decimais; aqui só pega sequência longa “pura”.
...
def sanitize_findings(findings: List[Dict[str, Any]], *, language: Language = "Português (Brasil)") -> Tuple[List[Dict[str, Any]], List[str]]:
